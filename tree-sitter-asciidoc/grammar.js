...
module.exports = grammar({
  ...
  rules: {
    ...
    author: $ =>
      seq(
        alias(repeat1($._author_line_word), $.firstname),
        $._WHITE_SPACE,
        optional(
          seq(
            alias(repeat1($._author_line_word), $.middlename),
            $._WHITE_SPACE,
          ),
        ),
        alias(repeat1($._author_line_word), $.lastname),
        optional(seq($._WHITE_SPACE, '<', $.email, '>')),
      ),
    _author_line_word: $ => escaped_ch(' ;', true),
    email: $ =>
